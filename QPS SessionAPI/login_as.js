...
app.get('/login', function (req, res) {
    ...
    var usr = req.query.usr;  // read querystring "usr" from url
    var dir = req.query.dir;
    var goto = (req.query.goto == 'undefined' ? '/hub' : req.query.goto);
        ...
        var sessionreq = https.request(options, function (sessionres) {
            ...
            sessionres.on('data', function (d) {
               ...
               // + "<script>window.location='/redirect?goto=" + goto + "';</script>" 
                + "Session set to  " + session.SessionId +" for user "+ session.UserId 
                + "Click <a href='/redirect?goto=" + goto + "'>here</a> if you are not redirected automatically."
                + "</body></html>");
                */
            });
        });
